...
export class FileIndexer {
            ...
            for (const [uuid, location] of Object.entries(autoSaveVirtualFolders)) {
                ...
                const lf = locationsToFolders.find(
                    (l) =>
                        ...
                        l.target === location.target &&
                        sameBucket(l.bucket, location.bucket),
                );
    ...
    static async saveFolderToCache(folder, notify = true) {
        if (!this._collection) {
            ui.notifications.warn(
                'Index was recently refreshed. Reload the browser before attempting to save the index.',
            );
            return;
        }
    ...
    static async generateIndex({
        ...
        for (let path of content.files) {
            ...
            if (file === 'noscan.txt') return null;
                ...
                if (
                    !(
                        cacheDir.target === dir.target &&
                        cacheDir.source === source &&
                        sameBucket(cacheDir.bucket, bucket)
                    )
                ) {
    ...
    static async _buildFauxForgeBrowser(source, dir) {
        ...
        if (
            source === 'forgevtt' ||
            !['modules', 'systems', 'worlds', 'assets'].includes(dir.replaceAll(/[\/\\]/g, ''))
        ) {
            paths = [dir];
        } else {
    ...
    static async _onAddDirectory() {
        this.selectFolder(async (selection) => {
            ...
            if (
                indexDirs.find(
                    (id) =>
                        id.source === selection.source &&
                        id.target === selection.target &&
                        sameBucket(id.bucket, selection.bucket),
                )
            ) {
                return;
            }
