...
export class PresetConfig extends foundry.applications.api.HandlebarsApplicationMixin(
    ...
    get title() {
        ...
        if (this.presets.length > 1) return `${prefix}s [${this.presets.length}]`;
            ...
            return `${prefix}: ${this.presets[0].name.substring(0, 20)}${this.presets[0].name.length > 20 ? '...' : ''}`;
    }
    ...
    static async _onAssignDocument() {
        const controlled = canvas
            .getLayerByEmbeddedName(this.presets[0].documentName)
            ?.controlled.map((p) => p.document);
