...
export class Spawner {
    ...
    static async spawnPreset({
        ...
        let presetAttached = preset.attached?.map((att) => {
            return { documentName: att.documentName, data: mergePresetDataToDefaultDoc(att, att.data) };
        });
    ...
    static _autoModifyData(docToData, hidden, flags, preserveLinks, sceneId) {
        hidden =
            hidden || game.keyboard.isModifierActive(foundry.helpers.interaction.KeyboardManager.MODIFIER_KEYS.ALT);
