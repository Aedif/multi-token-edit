...
export class PresetContainerV2 extends foundry.applications.api.HandlebarsApplicationMixin(
    ...
    _attachDragDropListeners(html) {
        html.on('dragstart', '.item', (event) => {
            ...
            const dragDropData = {
                uuids,
                sortable: itemList.find('.item.selected').hasClass('sortable'),
                type: 'preset',
            };
    ...
    _getItemContextOptions() {
        return [
            ...
            },
            {
                ...
                icon: '<i class="fa-solid fa-copy"></i>',
                condition: (item) =>
                    game.user.isGM && $(this.form).find('.item-list').find('.item.selected').length === 1,
        ...
        return [
            ...
            },
            {
                ...
                condition: (header) => {
                    ...
                    return (
                        folder.indexable &&
                        !game.settings.get(MODULE_ID, 'presetBrowser').autoSaveVirtualFolders?.[uuid]
                    );
                },
                callback: (header) => {
                    ...
                    const fp = new foundry.applications.apps.FilePicker.implementation({
                        ...
                        callback: (target, fp) => {
                            ...
                            settings.autoSaveVirtualFolders[uuid] = {
            ...
            },
            {
                ...
                condition: (header) => {
                    ...
                    const folder = fromUuidSync(uuid);
                    return (
                        folder.indexable && game.settings.get(MODULE_ID, 'presetBrowser').autoSaveVirtualFolders?.[uuid]
                    );
                },
                callback: (header) => {
                    const settings = game.settings.get(MODULE_ID, 'presetBrowser');
            ...
            },
            {
                ...
                condition: (header) => {
                    ...
                    const folder = fromUuidSync(uuid);
                    return (
                        !(folder instanceof VirtualFileFolder || folder instanceof PresetPackFolder) &&
                        !folder.collection.locked
                    );
                },
    ...
    async _folderExpand(folderElement, folder) {
        ...
        if (folderElement.find('.folder-items').length) {
            ...
            folderElement
                .find('header .folder-icon')
                .first()
                .removeClass('fa-folder-closed')
            ...
            const confirm =
                selected.length === 0
                    ...
                    : await Dialog.confirm({
                          ...
                          content: `<p>${localize('AreYouSure', false)}</p><p>${localFormat(
                              'presets.delete-presets-warn',
                              {
...
export function registerPresetDragDropHooks() {
    const canvasDrop = async function (canvas, point, event) {
    ...
    Hooks.on('dropCanvasData', canvasDrop);

    // Handle drag/drop on 3D Canvas
    Hooks.once('3DCanvasPreInit', (Levels3DPreview) => {
        const InteractionManager = Levels3DPreview.CONFIG.UI.InteractionManager;
        const _original_onDrop = InteractionManager.prototype._onDrop;

        InteractionManager.prototype._onDrop = async (event, dataTransfer) => {
            ...
            if (data?.uuids) {
                const coord3d = data.coord3d
                    ? game.Levels3DPreview.ruler.constructor.pos3DToCanvas(data.coord3d)
                    : game.Levels3DPreview.interactionManager.screen3DtoCanvas2DWithCollision(event);

                canvasDrop(canvas, coord3d, event);
                return;
            }

            return _original_onDrop.call(game.Levels3DPreview.interactionManager, event, data);
        };
    });
}
