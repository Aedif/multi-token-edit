...
export function registerKeybinds() {
    ...
    game.keybindings.register(MODULE_ID, 'smartUnlink', {
        ...
        onDown: () => {
            LinkerAPI.removeLinksFromSelected({
                notification: true,
                multiLayer: !Boolean(LinkerAPI._getSelected().length),
            });
        },
        restricted: true,
        precedence: CONST.KEYBINDING_PRECEDENCE.NORMAL,
        ...
        onDown: async (event) => {
            ...
            if (game.user.isGM) editing = await editPreviewPlaceables();
                ...
                if (
                    ...
                    canvas.tiles.controlled.every(
                        (t) => t.document.allowPlayerMove?.() && t.document.allowPlayerRotate?.(),
                    )
                ) {
    ...
    game.keybindings.register(MODULE_ID, 'tileToRegion', {
        name: 'Tile -> Region',
        hint: 'Non-alpha pixels of the selected tile textures will be used to create new regions.',
        editable: [],
        onDown: async (event) => {
            if (canvas.tiles.controlled.length) {
                for (const tile of canvas.tiles.controlled) {
                    tileToRegion(tile, { create: true, notification: true });
                }
            }
        },
        restricted: false,
