...
import { deleteEmbeddedDocumentsViaGM } from '../utils.js';
...
export class DeSpawnPresetBehaviorType extends foundry.data.regionBehaviors.RegionBehaviorType {
    ...
    async _handleRegionEvent(event) {
        ...
        if (this.all) {
            DeSpawnPresetBehaviorType.deSpawnAllPresets(
                this.parent.scene,
                this.originRegionOnly ? this.region.id : null,
            );
        } else {
            if (!this.presetUuids?.size) return;
            const uuids = Array.from(this.presetUuids);
            DeSpawnPresetBehaviorType.deSpawnPresets(
                uuids,
                this.parent.scene,
                this.originRegionOnly ? this.region.id : null,
            );
    ...
    static deleteEmbeddedDocuments(scene, embedName, ids) {
        deleteEmbeddedDocumentsViaGM(embedName, ids, {}, scene);
    }
}
