...
class LinkerMenu extends FormApplication {
    ...
    static onCreate(document) {
        if (canvas.scene.id === document.parent.id) this._processDocument(document);
    }
    ...
    static addLink(documentName, documentId, linkId, linkType, linkLabel) {
        ...
        let edge = graph
            .edges(documentId)
            .find((edge) => graph.source(edge) === linkId || graph.target(edge) === linkId);
    ...
    constructor() {
        ...
        const mePresetButton = ui.controls.element.querySelector('[data-control="me-presets"]');
        const pos = (mePresetButton ?? ui.controls.element).getBoundingClientRect();
        ...
        SUPPORTED_PLACEABLES.forEach((embeddedName) => {
            canvas.scene.getEmbeddedCollection(embeddedName).forEach((d) => {
                d.flags[MODULE_ID]?.links?.forEach((l1) => {
                    if (!links.find((l2) => l1.id === l2.id && l1.type === l2.type))
                        links.push(foundry.utils.deepClone(l1));
                });
            });
        });
    ...
    _addDocumentNode(documentName, documentId, doc = null, position = null) {
        ...
        const bounds = getDataBounds(documentName, document.toObject());
        const coords = position ?? { x: bounds.x + Math.random(), y: -bounds.y };
        ...
        if (graph.getNodeAttribute(node, 'isLink')) {
            ...
            if (this._selectedLinkNode === node) {
                ...
                if (this._selectedLinkNode)
                    graph.setNodeAttribute(this._selectedLinkNode, 'color', GRAPH_CONFIG.link.color);
